 ...
 *
 * 16개의 문장 생김새를 일부러 흩어 놓았다. 두 문장으로 끊기, 어미로 잇기, 결론을 앞에
 * 두기를 섞는다. 전에는 16개가 전부 "[상황] — [결론]" 한 틀이라, 조합을 두세 개만 돌려
 * 봐도 틀이 드러났다. 사람이 쓴 글은 줄마다 호흡이 다르다.
 */
const VESSEL_JOIN: Record<VesselCode, string> = {
  SROP: "가마솥이 이 성향을 만나면 불길이 더 커져요. 한 국자만 덜어두는 습관을 들이면 판이 계속 커집니다",
  SROJ: "무쇠솥이 이 성향을 만나면 달궈지는 속도가 빨라져서, 늦되는 그릇의 유일한 약점이 지워집니다",
  SRXP: "쓰임새만 정해주면 값이 뛰어요. 놋그릇에 광내는 손이 하나 생긴 셈입니다",
  SRXJ: "금고가 이 성향을 만나면 자물쇠에 다이얼이 하나 더 달려요. 가끔 여는 날만 정하면 은행이 됩니다",
  SrOP: "두레박이 이 성향을 만나면 퍼 올리는 손이 두 개가 돼요. 이제 부을 항아리 하나만 정하면 끝입니다",
  SrOJ: "뚝배기가 이 성향을 만나면 불 조절이 완벽해져서 식지 않는 구조가 완성됩니다",
  SrXP: "표주박이 이 성향을 만나면 물이 더 빨리 돌아요. 담는 통 하나만 두면 무적입니다",
  SrXJ: "옹달샘이 이 성향을 만나면 샘 주변이 넓어지고, 고이는 양부터 달라집니다",
  WROP: "소쿠리가 이 성향을 만나면 그물코가 촘촘해져요. 지나가던 큰돈이 걸리기 시작합니다",
  WROJ: "뚜껑 여는 타이밍을 알게 되는 조합이에요. 항아리가 곳간으로 가는 지름길입니다",
  WRXP: "유리병이 이 성향을 만나면 투명함이 무기가 돼서, 같이 벌자는 사람이 늘어납니다",
  WRXJ: "장독이 이 성향을 만나면 숙성 온도가 맞춰져요. 기다림이 그대로 장맛이 됩니다",
  WrOP: "화로가 이 성향을 만나면 불씨 값을 제대로 받게 돼요. 재주가 돈으로 바뀌는 지점입니다",
  WrOJ: "종지가 이 성향을 만나면 종지가 하나씩 늘어나고, 어느새 상이 달라집니다",
  WrXP: "대접이 이 성향을 만나면 베푼 것이 돌아오는 길이 넓어져요. 인심이 곧 자산이 됩니다",
  WrXJ: "달항아리가 이 성향을 만나면 안목에 가격표가 붙어요. 국보 감정이 시작됩니다",
};
